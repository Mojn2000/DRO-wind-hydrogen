...
# read Wasserstein distance based on wind power and day-ahead price forecast
df_Wdist = CSV.read("2-stage/dist2.csv", DataFrame, header=true)
...
# read Wasserstein distance based on all parameters
dist = CSV.read("2-stage/dist3.csv", DataFrame, header=true)
dist = Matrix(dist)

# DRO parameters
theta = 4   # Wasserstein radius
nS = 500    # number of decretization points
...
for block=t1:t2
    ...
    for s in 1:(nN+nS), n in 1:nN
        if s <= nN  # empirical sample
            lda_s = df[TT_daily[SN[s]], :spotMeas]  # lambda_DA for sample
            lb_s  = df[TT_daily[SN[s]], :imbalMeas] # lambda_B for sample
            pw_s  = df[TT_daily[SN[s]], :windMeas]  # wind power for sample  
...
CSV.write("Output/2-stage/DRO.csv", df_results)
