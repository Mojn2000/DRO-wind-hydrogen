...
#param_list = [0 30 60]
param_list = [1e-2 10 15 20 25 30]
param_list = [5]
#param_list = [1.5 2.0 4.0]
#param_list = [5 15 30]
#param_list = [20]
...
while it < length(param_list)
    ...
    for block=t1:t2
        ...
        function frobeniusInner(A,B)
            return sum( A .* B)
            #return sum(A[i,j] .* B[i,j] for i=1:size(A,1), j=1:size(A,2))
        end
        ...
        for t in 1:length(T)    
            ...
            #@constraint(m3s, [i=1:length_error[tt]], (F(t,i) .+ beta[t][i].*M) in PSDCone())
            #@constraint(m3s, [i=1:length_error[tt]], F(t,i) in PSDCone())

            # Wasserstein radius limit 
            #@constraint(m3s, [i=1:length_error[tt]], gamma[t][:,i] .>= -psi[t])
            #@constraint(m3s, [i=1:length_eror[tt]], gamma[t][:,i] .<=  psi[t])
        ...
        println(" RUNNING MEAN:  ", mean(df_results[df_results[:, :obj] .!= 0, :obj_active]))
        #println(" TARGET MEAN:  ",  mean_dro[k])
    end

    #CSV.write("Data/Output/1-stage/DRO-dependence/eta-"*string(eta)*".csv", df_results)
    CSV.write("Data/Output/1-stage/DRO-dependence/run-"*string(7)*"-eta-1.5.csv", df_results)
    #CSV.write("Data/Output/1-stage/DRO-dependence/train-"*string(it)*".csv", df_results)
    #CSV.write("Data/Output/1-stage/DRO-dependence/test-10000Sigma-"*string(theta)*".csv", df_results)
end


sum(df_results[!, :obj])
sum(df_results[!, :obj_active])
sum(df_results[!, :obj_passive])
